...
def section_comparison_chart(df):
    ...
    chart_data = top_10[[EVSchema.region, EVSchema.ev_count, EVSchema.charger_count, EVSchema.discomfort_index]]
    ...
    fig = px.bar(
        ...
        color_discrete_map={EVSchema.ev_count: 'blue', EVSchema.charger_count: 'orange'},
        labels={'value': '대수', 'variable': '구분'}
    )

    for _, row in chart_data.iterrows():
        fig.add_annotation(
            x=row[EVSchema.region], y=row[EVSchema.ev_count],
