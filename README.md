...
2. **충전 불편지수 산출 및 지도 표현**
   `(EV 등록수 × 가중치) + (민원수 × 가중치)` 공식으로 지수 산출 후
...
> 시연 이미지는 추후 업데이트 예정입니다.

| 연도별 전기차 등록 현황 |
|--|
|![연도별 전기차 등록 수와 충전기 수의 증가 추이를 비교하는 라인 차트 화면](docs/team_img.png) |
| 연도별 전기차 등록 수와 충전기 수의 증가 추이를 비교하는 라인 차트 화면입니다. |

| 지역별 SOS 핫스팟 지도 |
|--|
| ![17개 시도별 불편지수를 색상 농도로 표현한 핫스팟 지도](docs\sos.png) |
| 17개 시도별 불편지수를 색상 농도로 표현한 핫스팟 지도 화면입니다. |
